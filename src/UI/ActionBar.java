...
import Exceptions.InvalidTowerException;
...
public class ActionBar extends Bar {
    ...
    private void drawScoreInfo(Graphics g) {
        g.drawString("Score: " + score + " points", 110, 775);
    }
    ...
    private void drawDisplayedTower(Graphics g) {
        if(displayedTower != null) {
            ...
            g.drawImage(playing.getTowerManager().getTowerImages()[displayedTower.getTower_type()], 420, 650, 50, 50, null);
            g.setFont(new Font("Courier", Font.PLAIN, 15));
            g.drawString(Constants.Towers.GetName(displayedTower.getTower_type()), 480, 660);
    ...
    private int getUpgradeAmount(Tower displayedTower) {
        return (int)(Constants.Towers.GetTowerCost(displayedTower.getTower_type()) * 0.3f);
    }

    private int getSellAmount(Tower displayedTower) {
        int upgradeCost = (displayedTower.getTier() - 1) * getUpgradeAmount(displayedTower);
        upgradeCost *= 0.5f;
        return Constants.Towers.GetTowerCost(displayedTower.getTower_type()) / 2 + upgradeCost;
    }
    ...
    private void sellTowerClicked() {
        ...
        gold += Constants.Towers.GetTowerCost(displayedTower.getTower_type()) / 2;
    ...
    private void upgradeTowerClicked() throws InvalidTowerTypeException, InvalidTowerException {
    ...
    public void mouseClicked(int x, int y) {
        if (bMenu.getBounds().contains(x, y)) {
            SetGameState(MENU);
            DatabaseHandler.saveEnemies(playing.getEnemyManager().getEnemies());
            DatabaseHandler.saveTowers(playing.getTowerManager().getTowers());
            DatabaseHandler.saveGameState(playing.getLevelManager().getCurrentLevelIndex(), playing.getWaveManager().getWaveIndex(), playing.getWaveManager().getEnemyIndex(), getGold(), getLives());
            DatabaseHandler.insertScore(playing.getGame().getMenu().getPlayerName(), playing.getScore(), getLives(), getGold(), false);
            ...
            if (displayedTower != null) {
                if (sellTower.getBounds().contains(x, y)) {
                    ...
                    try {
                        upgradeTowerClicked();
                    } catch (InvalidTowerTypeException | InvalidTowerException e) {
